...
class Table {
    constructor(w, h) {
        ...
        this.lines.push({ x1: 5, y1: 5, x2: this.width - 5, y2: 5 });
        this.lines.push({ x1: this.width - 5, y1: this.height - 5, x2: this.width - 5, y2: 5 });
        this.lines.push({ x1: 5, y1: this.height - 5, x2: this.width - 5, y2: this.height - 5 });
        this.lines.push({ x1: 5, y1: this.height - 5, x2: 5, y2: 5 });
    }

    draw() {
        global.stroke(8, 64, 27);
        global.noFill();
        global.strokeWeight(3);
        for (let part of this.lines) {
            global.line(part.x1, part.y1, part.x2, part.y2);
        }
